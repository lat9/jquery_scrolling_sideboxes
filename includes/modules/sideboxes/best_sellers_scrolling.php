...
if (!isset ($_GET['products_id'])) {
  ...
  $check_query = "SELECT global_product_notifications FROM " . TABLE_CUSTOMERS_INFO . " WHERE customers_info_id = " . (int)$_SESSION['customer_id'] . " AND global_product_notifications = 1 LIMIT 1";
...
if ($show_best_sellers_scrolling) {
  ...
  if (isset ($current_category_id) && $current_category_id > 0) {
    $best_sellers_scrolling_query = "SELECT DISTINCT p.products_id, pd.products_name, p.products_ordered, p.products_image
  ...
  if ($best_sellers_scrolling->RecordCount() >= MIN_DISPLAY_BESTSELLERS) {
